...
class ViewEntity {
    ...
    constructor() {
        ...
        this.activationOffsetField =
            this.root.querySelector("#activation-offset");
    ...
    get priority() {
        if (
            this.priorityField.value == null ||
            this.priorityField.value.trim() == ""
        ) {
            return null;
        } else {
    ...
    get core() {
        if (
            this.coreField.value == null ||
            this.coreField.value.trim() == "Default"
        ) {
            return null;
        } else {
    ...
    get taskParametersClean() {
        ...
        return {
            ...
            priority:
                this.priority == null
                    ? null
                    : Math.abs(parseInt(this.priority, 10)),
            initialOffset:
                Math.abs(parseFloat(this.initialOffset)) * Utility.MsToNs,
            activationOffset:
                Math.abs(parseFloat(this.activationOffset)) * Utility.MsToNs,
    ...
    setupDeleteButtonListener(elementId) {
        const deleteButton = this.root.querySelector(
            `#${this.ElementIdPrefix}-${elementId}`
        );
    ...
    validateTaskParameters(taskParameters, core) {
        ...
        if (!Utility.ValidName(taskParameters.name.trim())) {
            alert(
                "Task name can only start with an alphabetical or underscore character, and continue with alphanumerical or underscore characters."
            );
            return false;
        }

        if (
            taskParameters.priority != null &&
            (isNaN(taskParameters.priority) ||
                parseInt(taskParameters.priority) < 0 ||
                taskParameters.priority.split(".").length != 1)
        ) {
            alert(
                "Priority has to be a positive integer. Lowest priority is 0."
            );
            return false;
        }

        if (
            taskParameters.initialOffset == null ||
            taskParameters.initialOffset.trim() == "" ||
            isNaN(taskParameters.initialOffset)
        ) {
        ...
        if (!Number.isSafeInteger(initialOffsetNs)) {
            alert(
                "Initial offset is unable to be represented with nanosecond precision."
            );
            return false;
        }

        if (
            taskParameters.activationOffset == null ||
            taskParameters.activationOffset.trim() == "" ||
            isNaN(taskParameters.activationOffset)
        ) {
        ...
        if (!Number.isSafeInteger(activationOffsetNs)) {
            alert(
                "Activation offset is unable to be represented with nanosecond precision."
            );
            return false;
        }
        ...
        if (
            ...
            taskParameters.duration.trim() == "" ||
            isNaN(taskParameters.duration)
        ) {
        ...
        if (!Number.isSafeInteger(durationNs)) {
            alert(
                "Duration offset is unable to be represented with nanosecond precision."
            );
            return false;
        }

        if (
            taskParameters.period == null ||
            taskParameters.period.trim() == "" ||
            isNaN(taskParameters.period)
        ) {
        ...
        if (!Number.isSafeInteger(periodNs)) {
            alert(
                "Period is unable to be represented with nanosecond precision."
            );
            return false;
        }
        if (activationOffsetNs + durationNs > periodNs) {
            alert(
                "Period is shorter than the combined activation offset and LET duration."
            );
            return false;
        }

        if (
            taskParameters.inputs == null ||
            taskParameters.inputs.trim() == ""
        ) {
            alert("Specify at least one input.");
            return false;
        }
        const inputs = taskParameters.inputs
            .split(",")
            .map((item) => item.trim())
            .filter(Boolean);
        ...
        if (duplicateInputs.length != 0) {
            alert(
                `Remove duplicate input ports: ${duplicateInputs.join(", ")}.`
            );
            return false;
        }
        const invalidInputs = inputs.filter(
            (input) => !Utility.ValidName(input)
        );
        if (invalidInputs.length > 0) {
            alert(
                "Input names can only start with an alphabetical or underscore character, " +
        ...
        if (
            taskParameters.outputs == null ||
            taskParameters.outputs.trim() == ""
        ) {
            alert("Specify at least one output.");
            return false;
        }
        const outputs = taskParameters.outputs
            .split(",")
            .map((item) => item.trim())
            .filter(Boolean);
        const duplicateOutputs = outputs.filter(
            (port, index, self) => self.indexOf(port) !== index
        );
        if (duplicateOutputs.length != 0) {
            alert(
                `Remove duplicate output ports: ${duplicateOutputs.join(", ")}.`
            );
            return false;
        }
        const invalidOutputs = outputs.filter(
            (output) => !Utility.ValidName(output)
        );
        if (invalidOutputs.length > 0) {
            alert(
                "Output names can only start with an alphabetical or underscore character, " +
                    "and continue with alphanumerical or underscore characters. " +
        ...
        const duplicatePortNames = inputsOutputs.filter(
            (port, index, self) => self.indexOf(port) !== index
        );
        if (duplicatePortNames.length != 0) {
            alert(
                `Remove input and output ports with same names: ${duplicatePortNames.join(
                    ", "
                )}.`
            );
            return false;
        ...
        if (
            taskParameters.wcet == null ||
            taskParameters.wcet.trim() == "" ||
            isNaN(taskParameters.wcet)
        ) {
        ...
        if (!Number.isSafeInteger(wcetNs)) {
            alert(
                "WCET is unable to be represented with nanosecond precision."
            );
            return false;
        }

        if (
            taskParameters.acet == null ||
            taskParameters.acet.trim() == "" ||
            isNaN(taskParameters.acet)
        ) {
        ...
        if (!Number.isSafeInteger(acetNs)) {
            alert(
                "ACET is unable to be represented with nanosecond precision."
            );
            return false;
        }

        if (
            taskParameters.bcet == null ||
            taskParameters.bcet.trim() == "" ||
            isNaN(taskParameters.bcet)
        ) {
        ...
        if (!Number.isSafeInteger(bcetNs)) {
            alert(
                "BCET is unable to be represented with nanosecond precision."
            );
            return false;
        }
        ...
        if (durationNs < wcetNs / core.speedup) {
            alert(
                `WCET cannot be greater than duration. \nWCET is ${
                    wcetNs / Utility.MsToNs / core.speedup
                } ms on core ${core.name} with a speedup of ${core.speedup}.`
            );
            return false;
        }
        if (wcetNs < bcetNs) {
        ...
        if (
            taskParameters.distribution == null ||
            taskParameters.distribution.trim() == ""
        ) {
    ...
    clearSelected() {
        this.taskSet
            ...
            .querySelectorAll("li")
            .forEach((item) => {
                item.classList.remove("taskSelected");
            });
    }
    ...
    draw(parentElement, taskParameters, speedup) {
        ...
        // Set up the canvas
        const anchor = parentElement.append("a");
        ...
        textInfo
            ...
            .attr("dy", "1.3em")
            .text(
                `WCET, ACET, BCET: ${taskParameters.wcet / Utility.MsToNs}, ${
                    taskParameters.acet / Utility.MsToNs
                }, ${taskParameters.bcet / Utility.MsToNs}`
            );
        textInfo
            .append("text")
            .attr("dy", "2.6em")
            .text(
                `Inputs: ${Utility.FormatTaskPorts(
                    taskParameters.name,
                    taskParameters.inputs
                )}`
            );
        textInfo
            .append("text")
            .attr("dy", "3.9em")
            .text(
                `Outputs: ${Utility.FormatTaskPorts(
                    taskParameters.name,
        ...
        textInfo
            ...
            .text(
                `Core: ${
                    taskParameters.core == null
                        ? ModelCore.Default.name
                        : taskParameters.core
                }`
            );
        textInfo
        ...
        graphInfo
            ...
            .attr(
                "x",
                scale(
                    taskParameters.initialOffset +
                        taskParameters.activationOffset
                )
            )
        ...
        graphInfo
            ...
            .attr(
                "x2",
                scale(taskParameters.initialOffset + taskParameters.period)
            )
            .attr("y1", View.BarHeight + View.BarMargin)
            .attr("y2", View.BarHeight + View.BarMargin)
            .attr("class", "period");
        ...
        graphInfo
            ...
            .attr(
                "transform",
                `translate(0, ${View.BarHeight + 2 * View.TickHeight})`
            )
            .call(x_axis)
    ...
    populateParameterForm(taskParameters) {
        ...
        this.priority =
            taskParameters.priority == null ? "" : taskParameters.priority;
        this.initialOffset = taskParameters.initialOffset / Utility.MsToNs;
        this.activationOffset =
            taskParameters.activationOffset / Utility.MsToNs;
        ...
        this.core =
            taskParameters.core == null ? "Default" : taskParameters.core;
    }

    formatTaskParametersInfo(taskParameters) {
        return `${taskParameters.name}: priority = ${
            taskParameters.priority
        }, initial offset = ${
            taskParameters.initialOffset
        }, activation offset = ${taskParameters.activationOffset}, duration = ${
            taskParameters.duration
        }, period = ${
            taskParameters.period
        }, inputs = ${Utility.FormatTaskPorts(
            taskParameters.name,
            taskParameters.inputs
        )}, outputs = ${Utility.FormatTaskPorts(
            taskParameters.name,
            taskParameters.outputs
        )}, wcet = ${taskParameters.wcet}, acet = ${
            taskParameters.acet
        }, bcet = ${taskParameters.bcet}, distribution = ${
            taskParameters.distribution
        }`;
    }
