...
class ViewExportImport {
    ...
    constructor() {
        ...
        this.platformCheckbox = this.root.querySelector(
            "#exportimport-platform"
        );
        this.inputsOutputsCheckbox = this.root.querySelector(
            "#exportimport-inputs-outputs"
        );
        this.tasksCheckbox = this.root.querySelector("#exportimport-tasks");
        this.dependenciesCheckbox = this.root.querySelector(
            "#exportimport-dependencies"
        );
        this.scheduleCheckbox = this.root.querySelector(
            "#exportimport-schedule"
        );
        this.eventChainsCheckbox = this.root.querySelector(
            "#exportimport-event-chains"
        );
        this.constraintsCheckbox = this.root.querySelector(
            "#exportimport-constraints"
        );
        ...
        this.importSystemSelector = this.root.querySelector(
            "#import-system-file"
        );
    ...
    set settings(settings) {
        ...
        if (settings.hasOwnProperty("selectedImporter")) {
            const importer = this.importerSystemDropdown
                .selectAll("a")
                .filter((element) => element == settings.selectedImporter);
            if (!importer.empty()) {
    ...
    setupImportSystemSelectorListener() {
        this.importSystemSelector.addEventListener("click", (event) => {
            ...
            const pluginImporter = Plugin.GetPlugin(
                Plugin.Type.Importer,
                this.importer
            );
        ...
        this.importSystemButton.addEventListener("click", (event) => {
            ...
            if (this.importSystemSelector.files.length < 1) {
                alert(
                    `${
                        pluginImporter.Name
                    } system importer requires a *.${pluginImporter.Input.toLowerCase()} file to be selected!`
                );
                return;
            }
            ...
            fileReader.onload = (event) => {
                pluginImporter
                    ...
                    .then((result) => handler(result, this.elementsSelected));
            };
        });
    }
    ...
    updateSystemImporters() {
        ...
        const nativePlugins = Plugin.OfTypeAndCategory(
            Plugin.Type.Importer,
            Plugin.Category.Native
        );
        const nativePlugin =
            Object.keys(nativePlugins).length > 0
                ? Object.keys(nativePlugins)[0]
                : null;
        const remainingPlugins = Object.keys(
            Plugin.OfType(Plugin.Type.Importer)
        );
        ...
        this.importerSystemDropdown
            ...
            .on("click", function (event, data) {
                thisRef.importerSystemDropdown
                    .node()
                    .querySelectorAll("a")
