...
class PluginImporterNative {
    // Plug-in Metadata
    static get Name() {
        return "LetSynchronise";
    }
    static get Author() {
        return "Eugene Yip";
    }
    static get Type() {
        return Plugin.Type.Importer;
    }
    static get Category() {
        return Plugin.Category.Native;
    }
    static get Input() {
        return Plugin.Input.Json;
    }
    static get Output() {
        return Plugin.Output.Json;
    }
    ...
    static async Result(rawSystem) {
        ...
        try {
            ...
            alert(
                `File to import does not use valid JSON: \n\n${error.message}`
            );
            return null;
        }
    ...
    static ValidateTaskDependencies(system) {
        ...
        if (
            ...
            alert(
                "File to import defines task dependencies but no input/output interface or tasks have been defined."
            );
            return false;
        }

        for (const dependency of dependencies) {
            const validationResult = this.ValidateTaskDependency(
                dependency,
                system
            );
    ...
    static ValidateTaskDependency(dependency, system) {
        ...
        if (
            dependency.source.task === undefined ||
            dependency.source.port === undefined
        ) {
            return `File to import has task dependency "${dependency.name}" with an undefined source port.`;
        }

        if (
            dependency.destination.task === undefined ||
            dependency.destination.port === undefined
        ) {
        ...
        if (dependency.source.task == Model.SystemInterfaceName) {
            let systemInterface = this.GetSystemInterface(
            ...
            if (
                task === null ||
                !task.outputs.includes(dependency.source.port)
            ) {
        ...
        if (dependency.destination.task == Model.SystemInterfaceName) {
            ...
            if (
                task === null ||
                !task.inputs.includes(dependency.destination.port)
            ) {
    ...
    static ValidateEventChains(system) {
        ...
        if (
            Object.keys(eventChains).length > 0 &&
            system[Model.DependencyStoreName] === undefined
        ) {
            alert(
                "File to import defines event chains but no task dependencies have been defined."
            );
            return false;
        }

        for (const eventChain of eventChains) {
            const validationResult = this.ValidateEventChain(
                eventChain,
                system
            );
            if (typeof validationResult == "string") {
    ...
    static ValidateEventChain(eventChain, system) {
        ...
        const dependencies = system[Model.DependencyStoreName];
        const validationResult = this.ValidateEventChainSegment(
            eventChain.name,
            eventChain,
            dependencies
        );
    ...
    static ValidateTimingConstraints(system) {
        ...
        if (
            ...
            system[Model.EventChainStoreName] === undefined
        ) {
            alert(
                "File to import defines constraints but no event chains have been defined."
            );
            return false;
        }

        for (const constraint of constraints) {
            const validationResult = this.ValidateConstraint(
                constraint,
                system
            );
            if (typeof validationResult == "string") {
