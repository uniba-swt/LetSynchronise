...
class PluginSchedulerFp {
    ...
    static async Result(makespan, executionTiming) {
        ...
        const tasksInstances = (await system[Model.EntityInstancesStoreName]).filter(entity => entity.type === 'task' && entity.value.length > 0);
        ...
        return Plugin.DatabaseContentsSet(system, systemElementSelected)
