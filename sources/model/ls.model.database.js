...
class ModelDatabase {
    ...
    constructor() {
        ...
        dbOpenRequest.onupgradeneeded = function (event) {
            ...
            if (event.oldVersion < 3) {
                this.db.createObjectStore(Model.NetworkDelayStoreName, {
                    keyPath: "name",
                    unique: true,
                });
                this.db.createObjectStore(Model.DeviceStoreName, {
                    keyPath: "name",
                    unique: true,
                });

                if (event.oldVersion < 2) {
                    this.db.createObjectStore(Model.CoreStoreName, {
                        keyPath: "name",
                        unique: true,
                    });
                    this.db.createObjectStore(Model.MemoryStoreName, {
                        keyPath: "name",
                        unique: true,
                    });
                    this.db.createObjectStore(Model.SystemInputStoreName, {
                        keyPath: "name",
                        unique: true,
                    });
                    this.db.createObjectStore(Model.SystemOutputStoreName, {
                        keyPath: "name",
                        unique: true,
                    });
                    this.db.createObjectStore(Model.DependencyStoreName, {
                        keyPath: "name",
                        unique: true,
                    });
                    ...
                    this.db.createObjectStore(Model.EntityStoreName, {
                        keyPath: "name",
                        unique: true,
                    });
                    this.db.createObjectStore(Model.EntityInstancesStoreName, {
                        keyPath: "name",
                        unique: true,
                    });
                    this.db.createObjectStore(Model.EventChainStoreName, {
                        keyPath: "name",
                        unique: true,
                    });
                    this.db.createObjectStore(
                    ...
                    this.db.createObjectStore(Model.ConstraintStoreName, {
                        keyPath: "name",
                        unique: true,
                    });
                    this.db.createObjectStore(
    ...
    getObjectStore(storeName, mode, promiseReject) {
        ...
        transaction.onerror = function (event) {
            promiseReject(
                console.error(
                    "ModelDatabase store error: " + event.target.errorCode
                )
            );
        };
    ...
    deleteSchedule = function (instancesStoreNames) {
        ...
        instancesStoreNames = instancesStoreNames.filter((name) =>
