...
class Utility {
    ...
    static LeastCommonMultipleOfArray(array) {
        const decimalPlaces = Utility.MaxOfArray(
            array.map((element) => Utility.DecimalPlaces(element))
        );
        const scaling = Math.pow(10, decimalPlaces);
        const arrayScaled = array.map((element) => element * scaling);

        return (
            arrayScaled.reduce(function (a, b) {
    ...
    static FormatDependencies(rawDependencies) {
        return rawDependencies.map((dependency) => {
            ...
            const sourceText =
                dependency.source.task == Model.SystemInterfaceName
            ...
            const destinationText =
                dependency.destination.task == Model.SystemInterfaceName
    ...
    static SortEntitiesInOrder(entities) {
        let sorted = [];
        sorted.push(entities.find((entity) => entity.name.includes("t0")));
        sorted.push(entities.find((entity) => entity.name == "t3"));
        sorted.push(entities.find((entity) => entity.name == "t1"));
        sorted.push(
            entities.find((entity) => entity.name.includes("encapsulation"))
        );
        sorted.push(entities.find((entity) => entity.name.includes("network")));
        sorted.push(
            entities.find((entity) => entity.name.includes("decapsulation"))
        );
        sorted.push(entities.find((entity) => entity.name == "t2"));
...
Utility.Interval = class {
    ...
    overlapsWith(other) {
        ...
        // |<--- other --->|              |<--- other --->|
        return (
            this.startTime < parseInt(other.endTime) &&
            this.endTime > parseInt(other.startTime)
        );
    }
